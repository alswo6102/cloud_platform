---
name: framework-list
description: List the current framework deployment presets, their behavior, default port, and suggested environment variable names. Use when the user asks which framework, language, or preset to choose.
access: read
plane: shared
---

# Framework List

Return the live preset catalog from the shared deployment preset module.
Do not guess which preset applies when multiple frameworks use the same language.
