...
- If you are missing a value, leave the field out and select the tool anyway. The dry-run collects what is needed, and selecting the operation is also what lets the console draw that operation's own form -- answering in prose instead leaves the user typing every field by hand. Reply in prose when no operation matches what they asked.
